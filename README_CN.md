...
第八，实验层最好同时支持**两条并行的工作线**：

- 一条更适合人类阅读、调试和手工修改代码的线，让研究者能真正看懂实验、定位问题、安心接手
- 一条更适合 AI 阅读、修改和扩展的线，可能会更偏向紧凑、约束明确的 CLI 形式

这个判断来自一个非常实际的观察：如果把所有实验都完全交给 AI，它有时像一个特别强的博士，有时又会像一个 10 岁小孩一样犯一些低级错误。但如果你先给 AI 一个**已经验证过的原型实验**，它通常就会变得非常擅长在这个基础上做改进和扩展。也就是说，人类可读的原型线提供了可信、可调试的基座，而面向 AI 的执行线则提供了更适合快速迭代的受约束接口。
